...
<script>
    ...
    $("#modal-profile-create").on("click", ".removeProfileDevice", function() {
        $(this).parents(".list-group-item").remove()
    });
    
    $("#modal-profile-create").on("click", "#addProfileDevice", function() {
        newDeviceHelperObj.callback = function(device) {
            $("#newProfileDeviceObjects").append(_createDeviceHtml(device))
        }
        $("#modal-helpers-newDeviceObj").modal("show")
    })

    $("#modal-profile-create").on("click", ".editProfileDevice", function() {
    ...
    $("#modal-profile-create").on("click", "#createProfileBtn", function() {
        ...
        if (hosts.length == 0) {
            ...
            makeToastr(JSON.stringify({
                state: "error",
                message: "Please hosts to create on"
            }));
            changeCreateProfileBox(1)
            return false;
        } else if (name == "") {
