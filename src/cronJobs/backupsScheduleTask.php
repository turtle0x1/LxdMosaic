...
foreach ($hostBackups as $hostId => $schedules) {
    foreach ($schedules as $schedule) {
        ...
        if (preg_match('/daily [0-9]{2}:[0-9]{2}/m', $schedule["scheduleString"])) {
            $tSchedule = new BackupSchedule("daily", [explode(" ", $schedule["scheduleString"])[1]]);
        } else {
        ...
        if ($tSchedule->getRange() == "daily") {
            foreach ($tSchedule->getTimes() as $time) {
                $task = $cronSchedule->run("$executeString $argString")->description($name . " daily@" . $time);
            ...
            foreach ($tSchedule->getDaysOfWeek() as $dayOfWeek) {
                foreach ($tSchedule->getTimes() as $time) {
                    $task = $cronSchedule->run("$executeString $argString")->description($name . " " . $dayOfWeekList[$dayOfWeek] . "@" . $time);
                    $task->timezone($timezone);
            ...
            foreach ($tSchedule->getTimes() as $time) {
                $tParts = explode(":", $time);
                $task = $cronSchedule->run("$executeString $argString")->description($name . " monthly@" . $time);
                $task->timezone($timezone);
