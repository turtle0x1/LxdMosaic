# # #!/usr/bin/env bash

dpkg --purge --force-all lxd lxc

apt-get update

apt-get install -y software-properties-common
add-apt-repository ppa:ondrej/php -y

# # Install Dependecies
apt-get install -y apache2 php8.4 php8.4-cli php8.4-mysql php8.4-xml php8.4-curl php8.4-mbstring unzip zip git nodejs openssl || exit $?
apt-get install -y mysql-server || exit $?
apt-get install -y --no-install-recommends cron || exit $?
apt-get install -y --no-install-recommends php8.4-xdebug || exit $?

mkdir -p src/sensitiveData/certs
mkdir -p src/sensitiveData/backups

chmod -R 777 src/sensitiveData/certs
chmod -R 777 src/sensitiveData/backups

npm install || exit $?

# Install Dependecies
/usr/bin/php8.4 /usr/bin/composer install || exit $?

cp .env.dist .env

sed -i -e "s/DB_HOST=.*/DB_HOST=localhost/g" .env
## DB User
sed -i -e "s/DB_USER=.*/DB_USER=root/g" .env
## DB Pass
sed -i -e "s/DB_PASS=.*/DB_PASS=root/g" .env

sed -i -e "s/LXD_CERTS_DIR=.*/LXD_CERTS_DIR=\/home\/runner\/work\/LxdMosaic\/LxdMosaic\/src\/sensitiveData\/certs\//g" .env

systemctl start mysql

mysql -u root -proot < sql/seed.sql
mysql -u root -proot < sql/0.1.0.sql
mysql -u root -proot < sql/0.2.0.sql
mysql -u root -proot < sql/0.3.0.sql
mysql -u root -proot < sql/0.5.0.sql
mysql -u root -proot < sql/0.6.0.sql
mysql -u root -proot < sql/0.7.0.sql
mysql -u root -proot < sql/0.9.0.sql

vendor/bin/phinx migrate -e mysql
